...
func TestWaterJugServiceImpl_SolveWaterJugProblem(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			args: args{2, 10, 4},
			want: []State{
				newState(2, 0, "Fill bucket X"),
				newState(0, 2, "Transfer bucket X to bucket Y"),
				newState(2, 2, "Fill bucket X"),
				newState(0, 4, "Transfer bucket X to bucket Y"),
			},
			wantBool:  true,
			wantError: nil,
		},
		{
			...
			args: args{4, 3, 2},
			want: []State{
				newState(4, 0, "Fill bucket X"),
				newState(1, 3, "Transfer bucket X to bucket Y"),
				newState(1, 0, "Empty bucket Y"),
				newState(0, 1, "Transfer bucket X to bucket Y"),
				newState(4, 1, "Fill bucket X"),
				newState(2, 3, "Transfer bucket X to bucket Y"),
			},
			wantBool:  true,
			wantError: nil,
		},
